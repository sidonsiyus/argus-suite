...
**Stage 5 · complete.** Every pixel integrates a real null geodesic of the
...
consequences of the integration. The render is HDR and graded through a
hand-rolled bloom pyramid, ACES, vignette, grain and chromatic aberration.
...
gargantua/
  ...
  shaders/post.bright.glsl       bright-pass extraction, soft knee
  shaders/post.blur.glsl         separable 9-tap Gaussian
  shaders/post.composite.glsl    bloom + ACES + vignette + grain + aberration
...
| `P` | control panel — 21 live parameters, presets, quality, paths |
| `A` | audio drone on / off |
| `S` | save a screenshot |
| `U` | copy a deterministic frame URL to the clipboard |
| `[` `]` | quality profile down / up |
| `Esc` | stop a cinematic path |
...
The physics pass now writes **linear HDR** into a half-float target; everything
tonal happens afterwards:

1. **bright pass** with a soft knee, so bloom fades in instead of banding
2. **four-level pyramid**, each level blurred separably then downsampled
3. **composite** — radial chromatic aberration, bloom, ACES, gamma, vignette,
   and finally grain

Two constraints from the brief pull against bloom, and both needed real work:

**Deep blacks.** Additive bloom bleeds into the shadow. At first pass the
horizon lifted to 21/255 — visibly milky. Weighting the pyramid toward its tight
levels got that to 4.3, still not black. The fix that actually works is to carry
the horizon mask out of the physics pass in the **alpha channel** and refuse to
add bloom where the photon fell in. The hole emits nothing, so no lens scatter
belongs inside it. The shadow now reads **exactly 0** at any bloom strength.

**Fine critical-curve detail.** The same wide bloom blunted the shadow edge from
a gradient of 220 down to 66. With the mask in place the edge holds at **191**,
and it actually sharpens as bloom rises, because the ring side brightens while
the shadow stays pinned at zero.

**Grain** is gated by luminance — `smoothstep(0.0, 0.055, luma)` — so it never
lifts a black pixel off the floor.

### Stage 5 — the application

**21 live parameters** in a control drawer, grouped physics / disk / sky / grade,
each wired straight to the shader uniform it drives.

**Four presets** — CINEMATIC, SCIENTIFIC, POSTER, POLAR — each setting all 21
values and a camera position.

**Three quality profiles**

| profile | steps | AA | DPR cap |
|---|---|---|---|
| LOW | 320 | 1 | 1.0 |
| MEDIUM | 700 | 1 | 1.5 |
| HIGH | 1200 | 2 | 2.0 |

**Four cinematic paths** — ORBIT, DIVE, POLAR, EDGE-ON — keyframed as
(radius, elevation, azimuth, time) and eased between, with OrbitControls
suspended while one runs and restored when it finishes.

**Persistence** — every parameter, the quality profile and the camera are saved
to `localStorage` and restored on load.

**Deterministic screenshot mode** — `U` copies a URL encoding the preset,
quality, all 21 parameters, the camera and the simulation time. Loading it
freezes the clock, hides the HUD and locks the camera, so the same URL always
renders the identical frame. Add `&dl=1` to have it save the PNG automatically.

**Audio** — an optional drone generated with the Web Audio API, no asset files.
Its pitch is derived from the orbital frequency at the disk's inner edge, so it
tracks the hole you are actually looking at; the filter and level follow camera
distance.

**Context-loss recovery** — the handler now disposes and rebuilds the render
targets and marks every material for recompilation, rather than resizing into
dead framebuffers.
...
### Stage 4 — the post chain

| check | bloom off | bloom on | verdict |
|---|---|---|---|
| shadow-centre max | 0 | **0** | pure black preserved ✓ |
| frame mean | 40.1 | 44.7 | bloom contributing **+11.4 %** ✓ |
| shadow-edge gradient | 219 | **191** | **87 %** of the sharp edge kept ✓ |

| component | measurement |
|---|---|
| grain | sky-patch std-dev 4.27 → 7.28 with grain enabled |
| vignette | corner brightness 70.6 → 11.9 at full strength |
| chromatic aberration | radial, zero at frame centre by construction |
| debug views 1–8 | bypass the grade entirely and render raw |

Graded composition at the default framing: black 4.4 % · sky 64.4 % ·
midtones 24 % · highlights 7.2 %, midtone rgb(91,76,56) at R/B **1.63**.

### Stage 5 — control

| check | result |
|---|---|
| parameters | **21** sliders, all driving their uniforms (verified both the physics pass and the post chain) |
| presets | four, all producing distinct parameter sets and cameras |
| quality profiles | steps 320 / 700 / 1200, AA 1 / 1 / 2, DPR cap 1.0 / 1.5 / 2.0 |
| cinematic paths | all four run and complete; DIVE travels r = 69.8 → 7.5, POLAR sweeps 86° → 60° elevation, ORBIT reaches 180° azimuth at halfway |
| persistence | 21 values written and restored across a reload |
| screenshot | PNG produced from the canvas |
| deterministic URL | reloads with clock frozen, HUD hidden, camera locked, parameters restored |
| audio | drone starts and stops cleanly |
| physics after all of it | shadow **0.224 %** from theory, horizon still **0** |
| console errors | **none** |
...
- Bloom is masked out of the horizon rather than physically scattered into it.
  A real lens would put *some* glow there; the brief asks for deep blacks, and
  the iconic image has a crisp shadow, so the mask wins.
