...
vec3 skyProbe(vec3 d) {
    ...
    // Deep, nearly black, with a faint violet wash. The bright blue-grey cage
    // this replaces was filling most of the frame and dragging the whole image
    // toward grey.
    vec3 base = mix(vec3(0.0011, 0.0008, 0.0028),
                    vec3(0.0034, 0.0020, 0.0068), 0.5 + 0.5 * d.y);
    float band = exp(-pow((asin(clamp(d.y,-1.0,1.0)) + 0.22) * 2.4, 2.0));
    base += vec3(0.0020, 0.0009, 0.0042) * band;   // soft nebula wash
    ...
    base += chk * vec3(0.0006, 0.0005, 0.0010);

    vec3 grid = mix(vec3(0.10, 0.30, 0.55), vec3(0.55, 0.40, 0.25),
                    0.5 + 0.5 * sin(lat * 3.0));
    return base + grid * line * uSkyGrid;
}

// Narkowicz's ACES filmic approximation — highlights roll off warm instead of
// clipping to white, which is what the beamed side of the disk needs.
