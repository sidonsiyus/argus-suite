...
vec3 shade(vec3 ro, vec3 rd) {
    ...
    if (uDebug == 9) return skyProbe(normalize(rd), 1.0);   // unlensed reference
...
void main() {
    ...
    // View 9 is the unlensed reference for view 0, so it has to go through the
    // identical tonemap — otherwise the A/B compares gamma against linear.
