...
// N days ago as YYYY-MM-DD in the console's timezone (for the history window).
export function daysAgoInTz(n, tz = process.env.CONSOLE_TZ || "Asia/Kolkata") {
  return new Date(Date.now() - Math.max(0, n) * 86400000).toLocaleDateString("en-CA", { timeZone: tz });
}
...
// Mark a message handled (or not) without replying, by toggling the standard
// \Answered flag — the same flag a reply sets, so "done" has one meaning
// everywhere (panel, checklist, and the mailbox itself).
export async function setAnswered(uid, done) {
  const id = await mailboxId();
  const body = { uids: [Number(uid)] };
  if (done) body.addFlags = ["\\Answered"];
  else body.removeFlags = ["\\Answered"];
  await hfetch(`/api/v1/mailboxes/${enc(id)}/folders/${enc(FOLDER)}/messages/flags`, { method: "POST", body });
  return true;
}
