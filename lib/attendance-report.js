...
// (a plain cat string is also accepted). Present/OD counted separately; every
// other category counts as Absent, matching the legacy P/OD/Absent roll-up.
export function computeDayStats(roster, entriesMap) {
  ...
  roster.forEach((r) => {
    ...
    const st = coarseStatus(cat);
    if (st === "od") od++;
    else if (st === "absent") {
      absent++;
      absentees.push({ ...r, cat, category: CAT_BY_KEY[cat]?.short || cat, reason: (e && e.reason) || "" });
    } else present++;
  });
...
export async function mhCockpitDocx(day, roster, statusMap, form) {
  ...
  const absRows = stats.absentees.length
    ? stats.absentees.map((s, i) => new TableRow({ children: [
        cell(tr(i + 1), { center: true, w: 700 }), cell(tr(s.full_name), { w: 3600 }), cell(tr(s.reg_no), { center: true, w: 2400 }),
        cell(tr(s.category), { center: true, w: 1400 }), cell(tr(s.reason), { w: 3000 }),
      ] }))
    : [new TableRow({ children: [cell(tr("—", { color: "888888" }), { center: true, w: 700 }), cell(tr("No absentees", { color: "888888" }), { w: 3600 }), cell(tr(""), { w: 2400 }), cell(tr(""), { w: 1400 }), cell(tr(""), { w: 3000 })] })];
  const absTable = new Table({ width: { size: 100, type: WidthType.PERCENTAGE }, borders, rows: [
    new TableRow({ tableHeader: true, children: [hcell("S.No", 700), hcell("Name of Absentee", 3600), hcell("Register Number", 2400), hcell("Category", 1400), hcell("Reason", 3000)] }),
    ...absRows,
  ] });
